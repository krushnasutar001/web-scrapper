...
echo ===============================
echo   Scralytics One-Click Launcher
echo ===============================
echo.
...
start "Scralytics Backend" cmd /c "set PORT=%BACKEND_PORT% && set FRONTEND_URL=http://localhost:%FRONTEND_PORT% && set DB_NAME=linkedin_automation && cd /d %BACKEND_DIR% && npm start"

echo Starting frontend on port %FRONTEND_PORT% ...
start "Scralytics Frontend" cmd /c "set PORT=%FRONTEND_PORT% && set REACT_APP_API_URL=http://localhost:%BACKEND_PORT% && cd /d %FRONTEND_DIR% && npm start"
